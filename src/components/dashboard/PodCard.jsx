...
export default function PodCard({ pod, onViewLogs }) {
  ...
  const gpuName =
    pod.machine?.gpuDisplayName || pod.gpuDisplayName || "Unknown GPU";
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex flex-col gap-3 hover:border-gray-700 transition-colors">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-500 font-mono">{pod.id}</span>
      </div>
        ...
        {isRunning ? (
          <>
            <ActionBtn
              icon={Square}
              label="Stop"
              onClick={() => handleAction(stopPod, "Stop")}
              disabled={busy}
            />
            <ActionBtn
              icon={RotateCcw}
              label="Restart"
              onClick={() => handleAction(restartPod, "Restart")}
              disabled={busy}
            />
            <ActionBtn
              icon={ScrollText}
          ...
          <>
            <ActionBtn
              icon={Play}
              label="Start"
              onClick={() => handleAction(startPod, "Start")}
              disabled={busy}
              variant="success"
            />
            <ActionBtn
